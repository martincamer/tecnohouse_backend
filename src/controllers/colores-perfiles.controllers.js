...
export const getColores = async (req, res, next) => {
  ...
  const result = await pool.query(
    "SELECT * FROM coloresPerfiles WHERE user_id = $1",
    [req.userId]
  );
...
export const getColor = async (req, res) => {
  const result = await pool.query(
    "SELECT * FROM coloresPerfiles WHERE id = $1",
    [req.params.id]
  );
...
export const actualizarColores = async (req, res) => {
  ...
  const result = await pool.query(
    "UPDATE coloresPerfiles SET color = $1 WHERE id = $2",
    [color, id]
  );
...
export const eliminarColor = async (req, res) => {
  const result = await pool.query("DELETE FROM coloresPerfiles WHERE id = $1", [
    req.params.id,
  ]);
