...
export const getCategorias = async (req, res, next) => {
  ...
  const result = await pool.query(
    "SELECT * FROM categoriasProductos WHERE user_id = $1",
    [req.userId]
  );
...
export const getCategoria = async (req, res) => {
  const result = await pool.query(
    "SELECT * FROM categoriasProductos WHERE id = $1",
    [req.params.id]
  );
...
export const actualizarCategoria = async (req, res) => {
  ...
  const result = await pool.query(
    "UPDATE categoriasProductos SET categoria = $1 WHERE id = $2",
    [categoria, id]
  );
...
export const eliminarCategoria = async (req, res) => {
  const result = await pool.query(
    "DELETE FROM categoriasProductos WHERE id = $1",
    [req.params.id]
  );
